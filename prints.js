...
function createGlitterParticles() {
  for (let i = 0; i < 50; i++) {
    ...
    blurOverlay.appendChild(particle);
  }
}
...
function openText(e) {
  ...
  setTimeout(createGlitterParticles, 100);
}
...
function closeText() {
  ...
  setTimeout(() => {
    blurOverlay
      .querySelectorAll(".glitter-particle")
      .forEach((p) => p.remove());
  }, 600);
}

expandBtn.addEventListener("click", openText);
closeBtn.addEventListener("click", closeText);

// Overlay schließen bei Klick auf Hintergrund
blurOverlay.addEventListener("click", (e) => {
  if (e.target === blurOverlay) closeText();
});

// Overlay schließen mit Escape-Taste
